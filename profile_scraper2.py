...
class ProfileScraper(Thread):
    ...
    def scrap_profile(self, profile_linkedin_url, profile_known_graduation_date):
        ...
        try:
            ...
            # Get all the job positions
            # try:
            #     job_positions = self.browser.find_element_by_id('education-section').find_elements_by_tag_name('li')
            # except:
            #     job_positions = []
            ...
            try:
                if (self.browser.execute_script("return (els = document.getElementsByClassName('lt-line-clamp__line').length)")):
                    profile_desc = self.browser.execute_script(
                    "return (function(){els = document.getElementsByClassName('lt-line-clamp__line');results = [];for (var i=0; i < els.length; i++){results.push(els[i].innerText);}return results;})()")
                               
                else:
                    profile_desc = self.browser.execute_script(
                    "return (function(){els = document.getElementsByClassName('lt-line-clamp__raw-line');results = [];for (var i=0; i < els.length; i++){results.push(els[i].innerText);}return results;})()")
            ...
            if len(job_positions) > 0:
                ...
                for job_no in len(job_positions)
